...
const DEFAULT_DPAPI_TIMEOUT_MS = 5000;
...
export function getImaRuntimeConfig({
  ...
  if (osName === 'windows') {
    ...
    return {
      ...
      paths: buildPaths({ appPath, appSupportDir, profileDir, knowledgeExtensionId, pathLib }),
      commands: {
        processPattern: env.IMA_PROCESS_PATTERN || pathLib.basename(appPath),
      },
      capabilities: {
        uiTransport: false,
        apiCookieDecryption: true,
      ...
      pending: [
        ...
        'Validate whether direct API still returns ima business error 600001 after DPAPI cookie decryption.',
      ],
    };
  }
...
export function getImaLocalStatePath() {
  return getImaRuntimeConfig().paths.localState;
}
...
export function quitImaApp() {
  const config = getImaRuntimeConfig();
  if (config.os === 'windows') {
    const imageName = getWindowsProcessImageName(config);
    try {
      execFileSync('taskkill', ['/IM', imageName, '/T', '/F'], {
        encoding: 'utf8',
...
export function isImaProcessRunning() {
  ...
  if (config.os === 'windows') {
    const imageName = getWindowsProcessImageName(config);
    try {
      const output = execFileSync('tasklist', ['/FI', `IMAGENAME eq ${imageName}`, '/NH'], {
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'ignore'],
        timeout: 2000,
      });
      return new RegExp(`\\b${escapeRegex(imageName)}\\b`, 'i').test(output);
    } catch {
      return false;
    }
  }
...
export function readImaCookieEncryptionKey({ timeoutMs = DEFAULT_DPAPI_TIMEOUT_MS } = {}) {
  const config = assertImaCapability('apiCookieDecryption', 'ima Chromium cookie decryption');
  if (config.os !== 'windows') {
    throw new Error(`ima cookie encryption key lookup is not implemented for ${config.label}.`);
  }
  if (!config.paths.localState || !fs.existsSync(config.paths.localState)) {
    throw new Error(`ima Chromium Local State was not found at ${config.paths.localState || '(empty)'}.`);
  }

  const script = String.raw`
$ErrorActionPreference = 'Stop'
Add-Type -AssemblyName System.Security
$localStatePath = [string]$env:OPENCLI_IMA_LOCAL_STATE
if ([string]::IsNullOrWhiteSpace($localStatePath)) {
  throw 'OPENCLI_IMA_LOCAL_STATE was not provided'
}
$state = Get-Content -Raw -LiteralPath $localStatePath | ConvertFrom-Json
$encryptedKeyText = [string]$state.os_crypt.encrypted_key
if ([string]::IsNullOrWhiteSpace($encryptedKeyText)) {
  throw 'Chromium Local State did not contain os_crypt.encrypted_key'
}
$encryptedKey = [Convert]::FromBase64String($encryptedKeyText)
$prefixLength = [Math]::Min(5, $encryptedKey.Length)
$prefix = [Text.Encoding]::ASCII.GetString($encryptedKey, 0, $prefixLength)
if ($prefix -eq 'DPAPI') {
  if ($encryptedKey.Length -le 5) {
    throw 'Chromium DPAPI key payload was empty'
  }
...
`;
  ...
  try {
    const output = execFileSync('powershell.exe', [
      ...
      env: {
        ...process.env,
        OPENCLI_IMA_LOCAL_STATE: config.paths.localState,
      },
      stdio: ['ignore', 'pipe', 'pipe'],
      timeout: timeoutMs,
      windowsHide: true,
    ...
    if (!key.length) throw new Error('DPAPI returned an empty Chromium cookie key.');
    return key;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`Could not decrypt ima Chromium cookie key through Windows DPAPI: ${message}`);
  }
}
...
function buildPaths({ appPath, appSupportDir, profileDir, knowledgeExtensionId, pathLib = path }) {
  return {
    ...
    localState: appSupportDir ? pathLib.join(appSupportDir, 'Local State') : '',
...
function getWindowsProcessImageName(config) {
  const pattern = String(config.commands.processPattern || '').trim();
  const fromPattern = pattern ? path.win32.basename(pattern) : '';
  const fromAppPath = config.paths.appPath ? path.win32.basename(config.paths.appPath) : '';
  return fromPattern || fromAppPath || 'ima.copilot.exe';
}
