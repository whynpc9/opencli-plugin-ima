...
function getRecentPreviewCandidateFiles() {
  const files = [];
  collectFiles(path.join(PROFILE_DIR, 'Sessions'), files, { recursive: false });
  collectFiles(path.join(PROFILE_DIR, 'shared_proto_db'), files, { recursive: false });

  try {
    for (const name of fs.readdirSync(PROFILE_DIR)) {
      if (!/^IMA_/i.test(name)) continue;
      const root = path.join(PROFILE_DIR, name);
