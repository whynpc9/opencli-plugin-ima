...
import {
  ...
  getImaCookieDb,
  getImaCookieHost,
  getImaLocalStatePath,
  getImaMmkvDir,
  getImaPreferencesPath,
  getImaRuntimeConfig,
  readImaCookieEncryptionKey,
...
export function inspectApiState() {
  ...
  const localState = getImaLocalStatePath();
  ...
  return {
    ...
    localState,
    localStateExists: Boolean(localState && fs.existsSync(localState)),
    sqliteAvailable,
...
function decryptCookieDb() {
  ...
  for (const row of rows) {
    ...
    material ||= getCookieDecryptionMaterial();
...
function getCookieDecryptionMaterial() {
  const runtime = getImaRuntimeConfig();
  if (runtime.os === 'windows') {
    const timeout = Number(process.env.IMA_DPAPI_TIMEOUT_MS || process.env.IMA_KEYCHAIN_TIMEOUT_MS || KEYCHAIN_TIMEOUT_MS);
    return {
      platform: 'windows',
      key: readImaCookieEncryptionKey({ timeoutMs: timeout }),
    };
  }

  return {
    platform: 'macos',
    password: getSafeStoragePassword(),
  };
}

function decryptChromeCookie({ encryptedHex, hostKey, material, password, metaVersion }) {
  let encrypted = Buffer.from(encryptedHex, 'hex');
  const prefix = encrypted.subarray(0, 3).toString('utf8');
  const decryption = material || { platform: 'macos', password };

  if (decryption.platform === 'windows') {
    return decryptWindowsChromeCookie({
      encrypted,
      hostKey,
      key: decryption.key,
      metaVersion,
  ...
  const key = crypto.pbkdf2Sync(decryption.password, 'saltysalt', 1003, 16, 'sha1');
  const iv = Buffer.alloc(16, 0x20);
  const decipher = crypto.createDecipheriv('aes-128-cbc', key, iv);
  const plaintext = Buffer.concat([decipher.update(encrypted), decipher.final()]);

  return stripChromeCookieHostDigest({ plaintext, hostKey, metaVersion }).toString('utf8');
}

function decryptWindowsChromeCookie({ encrypted, hostKey, key, metaVersion }) {
  const prefix = encrypted.subarray(0, 3).toString('utf8');
  if (!['v10', 'v11', 'v20'].includes(prefix)) {
    throw new Error(`Unsupported Windows Chromium cookie encryption prefix "${prefix || '(empty)'}".`);
  }

  const body = encrypted.subarray(3);
  if (body.length <= 28) {
    throw new Error('Windows Chromium cookie encrypted payload was too short.');
  }

  const algorithm = getAesGcmAlgorithm(key);
...
function getAesGcmAlgorithm(key) {
  if (Buffer.isBuffer(key) && key.length === 16) return 'aes-128-gcm';
  if (Buffer.isBuffer(key) && key.length === 32) return 'aes-256-gcm';
  throw new Error(`Unsupported Chromium AES-GCM key length: ${key?.length || 0} bytes.`);
}

function stripChromeCookieHostDigest({ plaintext, hostKey, metaVersion }) {
