...
import {
  getImaClientType,
  getImaRuntimeConfig,
  getKnowledgeExtensionId,
  launchImaForWebContents,
} from './platform.js';
import {
  findCachedAskSession,
  saveAskSession,
...
const DEFAULT_CDP_HTTP_TIMEOUT_MS = 3000;
...
export async function askImaWebContents({
  ...
  const requestedSessionMode = normalizeSessionMode(sessionMode, { hasSessionId: Boolean(requestedSessionId) });
  const needsKnowledgeBase = !requestedSessionId && requestedSessionMode !== 'continue';
  const knowledgeBase = needsKnowledgeBase || kb || kbId
    ? await resolveKnowledgeBaseWebContents({ kb, kbId })
    : { id: '', name: '' };
  const knowledgeBaseId = knowledgeBase.id;
  const session = await resolveAskSessionWebContents({
    requestedSessionId,
  ...
  saveAskSession({
    sessionId: session.sessionId,
    knowledgeBaseId: knowledgeBaseId || session.knowledgeBaseId || '',
    knowledgeBaseName: knowledgeBase.name || kb || '',
  ...
  return {
    status: 'success',
    knowledgeBase: knowledgeBase.name || session.knowledgeBaseName || kb || knowledgeBaseId,
    knowledgeBaseId: knowledgeBaseId || session.knowledgeBaseId || '',
    sessionId: session.sessionId,
    sessionMode: session.mode,
    modelType: resolvedModelType,
...
async function resolveAskSessionWebContents({ requestedSessionId = '', sessionMode = 'new', knowledgeBase = {} } = {}) {
  ...
  if (sessionMode === 'continue') {
    const cached = findCachedAskSession({
      knowledgeBaseId: knowledgeBase.id || '',
      knowledgeBaseName: knowledgeBase.name || '',
    });
    if (!cached?.sessionId) {
      const label = knowledgeBase.id || knowledgeBase.name
        ? ` for "${knowledgeBase.name || knowledgeBase.id}"`
        : '';
      throw new Error(`No cached ima ask session${label}. Start one with --session new or pass --session-id.`);
    }
    return {
      sessionId: cached.sessionId,
      mode: 'continue',
      knowledgeBaseId: knowledgeBase.id || cached.knowledgeBaseId || '',
      knowledgeBaseName: knowledgeBase.name || cached.knowledgeBaseName || '',
    };
  }

  const session = await initKnowledgeQaSessionWebContents({ knowledgeBaseId: knowledgeBase.id });
  return {
    sessionId: session.sessionId,
    mode: 'new',
    knowledgeBaseId: knowledgeBase.id || '',
...
function normalizeSessionMode(value, { hasSessionId = false } = {}) {
  const mode = String(value || '').trim().toLowerCase();
  if (!mode) return hasSessionId ? 'continue' : 'new';
  if (mode === 'current') return 'continue';
  if (['new', 'continue'].includes(mode)) return mode;
  throw new Error('Session mode must be one of: new, continue.');
}
...
export async function inspectImaWebContentsTargets() {
  const port = getCdpPort();
  await ensureImaCdp(port);
  const targets = await fetchCdpJson(port, '/json/list');
  const selected = selectImaWebContentsTarget(targets);
  return {
    status: 'success',
    port,
    targetCount: targets.length,
    selectedTarget: selected ? sanitizeTarget(selected) : null,
...
function buildPostJsonExpression({ endpoint, payload, extensionVersion, clientType = 'mac', timeoutMs = DEFAULT_CDP_TIMEOUT_MS }) {
  return `
(async () => {
  const endpoint = ${JSON.stringify(endpoint)};
  const payload = ${JSON.stringify(payload)};
  const extensionVersion = ${JSON.stringify(extensionVersion)};
  const clientType = ${JSON.stringify(clientType)};
  ...
  try {
    const headers = await buildImaHeaders(extensionVersion, clientType);
...
function buildAskExpression({ endpoint, payload, extensionVersion, clientType = 'mac', timeoutMs }) {
  return `
(async () => {
  const endpoint = ${JSON.stringify(endpoint)};
  const payload = ${JSON.stringify(payload)};
...
const buildImaHeaders = async (extensionVersion, clientType = 'mac') => {
...
const collectStructuredText = (value, output) => {
  if (!value) return;
  if (Array.isArray(value)) {
...
const extractStructuredBlockText = (value) => {
  const parts = [];
...
const collectQaStream = async (response) => {
  ...
  for (;;) {
    ...
    for (const message of messages) {
      ...
      } else if (event.event === 'STRUCTURED_BLOCK') {
...
function selectImaWebContentsTarget(targets) {
  ...
  const knowledgeExtensionId = getKnowledgeExtensionId();
  return candidates.find((target) => target.url === 'chrome://allknowledge/') ||
    candidates.find((target) => target.url?.startsWith(`chrome-extension://${knowledgeExtensionId}/`)) ||
...
async function fetchCdpJson(port, endpoint) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), DEFAULT_CDP_HTTP_TIMEOUT_MS);
  try {
    ...
    if (error?.name === 'AbortError') {
      throw new Error(`CDP HTTP ${endpoint} timed out after ${Math.round(DEFAULT_CDP_HTTP_TIMEOUT_MS / 1000)}s`);
    }
    throw error;
  } finally {
    clearTimeout(timer);
